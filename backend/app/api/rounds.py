...
def create_round(company_id: int, payload: RoundCreate, db: Session = Depends(get_db)):
...
def list_rounds(company_id: int, db: Session = Depends(get_db)):
...
def get_round(company_id: int, round_id: int, db: Session = Depends(get_db)):
...
def update_round(company_id: int, round_id: int, payload: RoundUpdate, db: Session = Depends(get_db)):
...
def delete_round(company_id: int, round_id: int, db: Session = Depends(get_db)):
